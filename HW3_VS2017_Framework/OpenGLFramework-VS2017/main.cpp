	...
	if (mouse_pressed) {
		if (starting_press_x < 0 || starting_press_y < 0) {
			...
			case LightPositionChange:
				if (cur_LightMode == 0)
				{
					directionalInfo.LightPosition = directionalInfo.LightPosition + Vector3(0.005 * (diff_x), -0.005 * (diff_y), 0);
				}
				if (cur_LightMode == 1)
				{
					pointInfo.LightPosition = pointInfo.LightPosition + Vector3(0.005 * (diff_x), -0.005 * (diff_y), 0);
				}
				if (cur_LightMode == 2)
				{
					spotInfo.LightPosition = spotInfo.LightPosition + Vector3(0.005 * (diff_x), -0.005 * (diff_y), 0);
				}
				break;
			case ShineeShange:
				break;
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "109062530  HW3", NULL, NULL);
    if (window == NULL)
    {
